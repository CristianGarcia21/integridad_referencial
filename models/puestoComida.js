...
const puestoComidaSchema = new mongoose.Schema({
    id_puesto_comida: { type: Number, required: true, unique: true },
...
puestoComidaSchema.pre("save", async function (next) {
    ...
    for (let id of this.idMenu) {
      const menuExists = await Menu.findOne({ id_menu: id });
      if (!menuExists) {
...
puestoComidaSchema.post("findOneAndUpdate", async function (doc) {
  if (doc) {
    console.log(`Puesto de comida actualizado: ${doc.id_puesto_comida}`);
  }
});
...
puestoComidaSchema.pre("remove", async function (next) {
  ...
  console.log(`Puesto de comida eliminado: ${this.id_puesto_comida}`);
  next();
});
