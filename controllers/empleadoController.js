  ...
  try {
    ...
    if (!Array.isArray(empleados)) {
      return res
        .status(400)
        .json({
          error: "El cuerpo de la solicitud debe ser un arreglo de empleados.",
        });
    }
    ...
    for (const empleado of empleados) {
      try {
        ...
        if (!_id || !nombre || !apellido || !telefono || !direccion || !rol) {
          throw new Error(
            `Faltan campos obligatorios para el empleado con ID ${
              _id || "desconocido"
            }.`
          );
        }
        ...
        if (!rolesValidos.includes(rol)) {
          throw new Error(
            `Rol inválido para el empleado con ID ${_id}: debe ser uno de ${rolesValidos.join(
              ", "
            )}.`
          );
        }
...
exports.eliminarEmpleado = async (req, res) => {
  try {
    ...
    const sesionConEmpleado = await Sesion.findOne({
      ced_empleado: empleadoId,
    });
    ...
    if (!resultado) {
      return res
        .status(404)
        .json({ error: `Empleado con ID ${empleadoId} no encontrado.` });
    }

    res
      .status(200)
      .json({ message: `Empleado con ID ${empleadoId} eliminado.` });
  } catch (error) {
    console.error(error);
    res.status(500).json({ error: error.message });
  }
...
exports.actualizarEmpleado = async (req, res) => {
  ...
  try {
    ...
    if (!empleado) {
      return res
        .status(404)
        .json({ error: `Empleado con ID ${id} no encontrado.` });
    }
    ...
    // Actualizar referencias en `EmpleadoXCarro`
    if (datosActualizados._id) {
      await EmpleadoXCarro.updateMany(
        { ced_emp: id },
        { $set: { ced_emp: datosActualizados._id } }
      );
    }
